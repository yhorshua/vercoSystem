...
import { scanItem, closePacking, getScanStatus } from '../services/packingService'; // Asegúrate de importar correctamente
...
const marcarPorCodigo = async (codigoBarras: string) => {
  ...
  if (!item) {
    return; // Si no se encuentra el artículo, salimos
  }

  const cantidadSolicitada = item.cantidades[talla] || 0; // Cantidad solicitada de esa talla
  const cantidadActual = pickingStatus[codigo]?.[talla] || 0; // Cantidad escaneada de esa talla

  console.log("Código:", codigo);
  console.log("Talla:", talla);
  console.log("Cantidad solicitada:", cantidadSolicitada);
  console.log("Cantidad escaneada:", cantidadActual);

  // Verifica si la cantidad escaneada es menor que la cantidad solicitada
  if (cantidadActual < cantidadSolicitada) {
    try {
      // Consultar si ya se ha escaneado esta talla
      const scanStatus = await getScanStatus(Number(pedido.id), user.token);

      // Buscar el estado del artículo (código + talla) en los escaneos
      const scanItemStatus = scanStatus.find((item: any) => item.codigo === codigo && item.talla === talla);

      // Si ya se ha escaneado completamente, mostrar advertencia
      if (scanItemStatus && scanItemStatus.escaneado >= scanItemStatus.solicitado) {
        Swal.fire({
          icon: 'warning',
          title: 'Artículo ya escaneado completamente',
          text: `El artículo ${codigo}, talla ${talla}, ya ha sido escaneado por completo.`,
          timer: 4000,
          showConfirmButton: false,
        });
        return; // Si ya está completo, no registrar el escaneo
      }
      ...
      setPickingStatus((prev) => ({
        ...
        [codigo]: {
          ...prev[codigo],
          [talla]: cantidadActual + 1, // Incrementamos el conteo de escaneos para esa talla
        }
      }));
    } catch (error: any) {
      Swal.fire({
  ...
  const pickingCompleto = pedido.items.every(item =>
    Object.entries(item.cantidades).every(([talla, cantidadSolicitada]) => {
      const leido = pickingStatus[item.codigo]?.[Number(talla)] || 0;
      const completo = leido === cantidadSolicitada;
      console.log(`Item ${item.codigo}, Talla: ${talla}, Solicitado: ${cantidadSolicitada}, Leído: ${leido}, Completo: ${completo}`);
      return completo;
    })
  );
  console.log("pickingCompleto:", pickingCompleto);
